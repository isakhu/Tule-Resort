...
export default function ServiceEditor({ initial, onSaved, onCancel }: {
  ...
  async function save() {
    ...
    const result = form.id ? await supabase.from('resort_services').update(payload).eq('id', form.id) : await supabase.from('resort_services').insert(payload)
    setSaving(false)
  ...
  return (
    <div className="space-y-5 rounded-2xl border border-[#0B3D4A]/10 bg-white p-5 shadow-sm">
      ...
      <label className="block space-y-1"><span className="field-label">Image URL</span><input className="field-input" placeholder="Paste a Supabase/public image URL" value={form.image_url} onChange={e => set('image_url', e.target.value)} /></label>
