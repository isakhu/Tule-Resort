...
import { ArrowLeft, ChevronLeft, ChevronRight, Heart, Minus, Plus, ShoppingBag, Sparkles, Utensils } from 'lucide-react';
...
function FoodCard({ item, quantity, liked, accent, onAdd, onRemove, onLike }: { item: MenuItem; quantity: number; liked: boolean; accent: string; onAdd: () => void; onRemove: () => void; onLike: () => void }) {
  return (
    <article className="snap-center shrink-0 w-[min(86vw,420px)] md:w-[420px] h-[72vh] min-h-[540px] max-h-[760px] overflow-hidden rounded-[2rem] relative bg-[#15130F] shadow-[0_28px_70px_rgba(0,0,0,0.38)] border border-white/10">
      ...
      <div className="absolute top-20 left-5 rounded-2xl bg-white/95 px-4 py-3 shadow-xl"><p className="text-[9px] font-black uppercase tracking-[.18em] text-stone-500">Price</p><p className="text-2xl leading-none font-black text-stone-950">{item.price} <span className="text-xs align-middle">ETB</span></p></div>
      <div className="absolute left-5 right-5 bottom-5">
        <div className="mb-4 max-w-[92%]"><p className="mb-1 text-[11px] font-bold uppercase tracking-[.24em]" style={{ color: accent }}>{item.amharicName}</p><h2 className="text-4xl md:text-5xl font-black uppercase leading-[.95] tracking-[-.045em] text-white">{item.name}</h2></div>
...
export default function VisualResortMenu({ serviceName }: { serviceName: string }) {
  ...
  const totalPrice = useMemo(() => menuItems.reduce((sum, item) => sum + item.price * (cart[item.id] ?? 0), 0), [cart]);
